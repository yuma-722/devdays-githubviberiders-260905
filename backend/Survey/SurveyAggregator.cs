...
public static class SurveyAggregator
{
    ...
    {
        Dictionary<string, int> communities = SurveyOptions.Communities
            .Append(SurveyOptions.Unaffiliated).ToDictionary(value => value, _ => 0, StringComparer.Ordinal);
        ...
        foreach (SurveyDocument survey in surveys)
        {
            ...
            {
                communities[SurveyOptions.Unaffiliated]++;
            }
            foreach (string community in survey.CommunityAffiliation.Distinct(StringComparer.Ordinal))
            {
        ...
        return new SurveyResults(surveys.Count, communities, roles,
...
public static class SurveyDocumentValidator
{
    public static void Validate(SurveyDocument survey)
    {
        if (string.IsNullOrWhiteSpace(survey.Id)
            ...
            || survey.CommunityAffiliation is null || survey.JobRole is null || survey.Feedback is null)
        {
        ...
        try
        {
            SurveyRequestParser.ValidateRules(
                survey.CommunityAffiliation, survey.JobRole, survey.JobRoleOther, survey.EventRating, survey.Feedback);
        }
