...
「東京の路線図」をモチーフに、4 つの設問を縦のレール上の駅として並べ、回答が済むほどレールが塗り進みます。
...
- `communityAffiliation`: 配列（空可）。「どちらでもない」を選ぶと空配列で送信
...
| コミュニティ | `group` | `所属コミュニティ` |
| コミュニティ選択肢 | `checkbox` | `VS Code Meetup` / `GitHub dockyard` / `どちらでもない` |
| 職種 | `group` | `職種` |
